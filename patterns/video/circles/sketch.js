...
// `scl` - Scale multiplier (10) (30) becomes abstract, great pixel-like texture
...
function setup() {
  ...
  // Canvas size = 64 * 10 = 640 pixels wide, 48 * 10 = 480 pixels tall
...
function draw() {
  ...
  for (let i = 0; i < vid.width; i++) {
    ...
    for (let j = 0; j < vid.height; j++) {
      ...
      // `i * scl` - X position: column index scaled up (0-63 becomes 0-630)
      // `j * scl` - Y position: row index scaled up (0-47 becomes 0-470)
