// ============================================
// OPTIMIZED PERFORMANCE VERSION (Auto-start)
// Lower resolution, faster rendering, no button
// ============================================
...
function setup() {
  ...
  console.log('🚀 OPTIMIZED version (auto-start) loaded');
  console.log(`Resolution: ${w}x${h} | FPS: ${TARGET_FPS}`);

  // Auto-start audio (no button needed)
  startAudio();
}
...
function startAudio() {
  if (!audioStarted) {
    ...
    console.log('🎵 Play any audio on your Mac - captured via BlackHole!');
  }
}
...
function draw() {
  ...
  const sizeMod = (i, j) => {
    let charSize = baseCharSize * effects.bassPulse;
