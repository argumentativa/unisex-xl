// ============================================
// MICROPHONE VERSION (Refactored)
// Audio-reactive using microphone input
// ============================================
...
function setup() {
  ...
  console.log('🎙️ Click button to start microphone!');
}
...
function startAudio() {
  if (!audioStarted) {
    ...
    console.log('🎵 Make some noise - speak, play music, clap!');
  }
}
  ...
  if (audioStarted && fft) {
    ...
    if (frameCount % 30 === 0) {
      ...
      if (bassSmooth < 5 && mid < 5 && treble < 5) {
        console.warn('⚠️ NO AUDIO DETECTED! Try making noise!');
      }
    }
...
function drawAudioDebug() {
  ...
  textAlign(LEFT, TOP);
  textStyle(BOLD);
  text('🎙️ AUDIO LEVELS (Microphone)', 20, height - 140);
