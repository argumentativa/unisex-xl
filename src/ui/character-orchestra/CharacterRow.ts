 ...
 * Row containing 8 step buttons for one character
 */
...
export class CharacterRow {
  ...
  constructor(
    ...
    // Create 8 step buttons with callbacks
    for (let i = 0; i < 8; i++) {
  ...
  updateStep(stepIndex: number, stepState: StepState): void {
    if (stepIndex >= 0 && stepIndex < 8 && stepIndex < this.stepButtons.length) {
  ...
  updateCurrentStep(stepIndex: number): void {
    ...
    if (this.currentStep >= 0 && this.currentStep < 8 && this.currentStep < this.stepButtons.length) {
